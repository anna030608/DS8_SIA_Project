...
# config 로드 (가중치 + Level2 패널티)
config_path = os.path.join(os.path.dirname(__file__), '..', '..', 'config.yaml')
with open(config_path, encoding="utf-8") as f:
    config = yaml.safe_load(f)

w = config['priority_score']['weights']
level2_penalty = config['priority_score'].get('geo_level2_penalty', 0.5)
...
# ── 반복(모호) 좌표 목록 — GDELT 국가/지역 대표 좌표 ──────
# 실제 군사 현장이 아닌 행정·지역 중심 좌표 (방식 B: 10개로 확장)
AMBIGUOUS_COORDS = [
...
# ── Level2 패널티 적용 + priority_score 재계산 ───────────
# geo_level별 가중치: Level1 그대로, Level2 패널티(0.5), Level3는 어차피 제외
penalty_map = {'Level1': 1.0, 'Level2': level2_penalty, 'Level3': 0.0}
df['score_geo'] = df['score_geo'] * df['geo_level'].map(penalty_map)

# priority_score 최종 재계산 (패널티 반영된 score_geo + zscore 포함)
df['priority_score'] = (
...
# ── 저장: Level3 제외, Level1/2 포함 ─────────────────────
df_geo = df[df['geo_level'] != 'Level3'].copy()
df_geo = df_geo.sort_values('priority_score', ascending=False)

df_geo.to_csv("project/01_data/processed/final_priority_geo.csv", index=False)
print(f"\nLevel2 패널티({level2_penalty}) 적용 + priority_score 재계산 완료")
print(f"저장 완료: {len(df_geo)}행 → final_priority_geo.csv")
print(f"최종 Score 범위: {df_geo['priority_score'].min():.3f} ~ {df_geo['priority_score'].max():.3f}")