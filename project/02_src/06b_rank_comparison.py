"""
07_rank_comparison_v2.py
----------------------------------------------------------------------
정답 위기 사건이 가중치 5개 버전별로 '전체 몇 위'에 오는지 비교.
- 대표 행: 각 위기 날짜(±3일) 안에서 NumMentions 최대 사건 행 (고정)
...
# ── 비교할 가중치 5개 버전 ───────────────────────────────
# [geo, mentions, zscore, goldstein, tone]
WEIGHT_VERSIONS = {
...
def main():
    ...
    for start, end, name, importance in KNOWN_CRISES:
        ...
        r = df.loc[idx]
        row = {
            '사건': name, '중요도': importance, '날짜': start,
    ...
    print(f"{'='*80}")
    print(f"정답 사건 순위 비교 — 가중치 5개 버전 (전체 {total_events}건)")
    print(f"{'='*80}")
    print("버전 정의:")
    for vname, w in WEIGHT_VERSIONS.items():
        print(f"  {vname}: geo={w['geo_distance']} men={w['num_mentions']} "
              f"z={w['zscore']} gold={w['goldstein']} tone={w['avg_tone']}")
    print(f"대표 행: 각 위기 날짜 ±3일 내 NumMentions 최대 사건")
    print()
    ...
    # mentions 내림차순 정렬
    result_sorted = result.sort_values('mentions', ascending=False, na_position='last')
    ...
    result.to_csv("project/01_data/processed/rank_comparison_v2.csv", index=False, encoding='utf-8-sig')
    print("저장: project/01_data/processed/rank_comparison_v2.csv")
    print(f"{'='*80}")
