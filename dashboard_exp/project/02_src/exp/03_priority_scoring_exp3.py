...
total = len(df)
print("=" * 100)
print("이벤트별 Priority Score 상위 N% 비교 (낮을수록 좋음)")
print("=" * 100)
print(f"{'이벤트':<30}", end="")
...
# ── Recall / Precision / F1 검증 ─────────────────────────
print()
print("=" * 100)
print(f"Recall / Precision / F1 검증 (버퍼 ±{BUFFER_DAYS}일, 유효 위기 날짜: {len(actual)}일)")
print("=" * 100)
print(f"{'방식':<14} {'Precision':>10} {'Recall':>8} {'F1':>8} {'TP':>5} {'FP':>5} {'FN':>5} {'최적 Threshold':>15}")
print("-" * 80)
...
for name in WEIGHT_SETS:
    ...
    print(f"{name:<14} {best_p:>10.3f} {best_r:>8.3f} {best_f1:>8.3f} {best_tp:>5} {best_fp:>5} {best_fn:>5} {best_thr:>15.2f}")
    results.append({
...
print()
print(f"F1 최고: {best_row['name']} — Precision={best_row['precision']:.3f} / Recall={best_row['recall']:.3f} / F1={best_row['f1']:.3f} / Threshold={best_row['threshold']:.2f}")
df_results.to_csv("project/01_data/processed/weight_validation_results.csv", index=False)
print("저장 완료: project/01_data/processed/weight_validation_results.csv")